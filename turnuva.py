# turnuva.py (Kalıcı Kayıt Özellikli)
...
def get_initial_fixture():
    """Turnuvanın başlangıç fikstürünü döndürür."""
    return [
...
# Verileri session state'e yükle (sadece ilk çalıştırmada)
...
def calculate_stats():
    ...
    for match in st.session_state.fixture:
        if match.get('played', False): # .get() ile 'played' anahtarı olmayan eski kayıtlarda hata vermesini engelle
...
st.title("🏆 PES 2v2 Turnuva Paneli (Kalıcı Kayıt) 🏆")
st.markdown("---")
...
with col1:
    ...
    for i, match in enumerate(st.session_state.fixture):
        ...
        # Eğer skor None ise 0 göster, değilse kayıtlı skoru göster
        ...
        if row[5].button("Kaydet", key=f"save_{i}"):
            ...
            save_data(st.session_state.fixture)  # Değişiklikleri dosyaya kaydet
            st.rerun()
